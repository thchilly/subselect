...
|---|---|---|---|
| `GR_model_performance_HM.ipynb` | Annual + seasonal HPS pipeline (TSS, BVS, harmonic mean) for Greece. Reads CMIP6 from `Data/CMIP6/monthly_new/` and obs from `Data/ISIMIP3a/monthly_cmip6_upscaled/`. | `subselection_paper/performance_figures/` (Taylor diagrams, HPS tables) | `subselect.performance` (M7) + `subselect.viz.taylor`, `subselect.viz.performance_figs` (M9) |
